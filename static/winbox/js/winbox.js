...
function WinBox(params, _title) {
  ...
  if (params.root === undefined) { index || setup(); } else { index || setup(params.root); }
...
function setup(rootElement = body) {
  body = document.body;
...
function init(rootElement = body) {
  ...
  root_w = rootElement.clientWidth;
  root_h = rootElement.clientHeight;
}
